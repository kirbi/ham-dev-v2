<div>
    ...
    <form wire:submit.prevent="save">
        ...
        <div class="mb-4">
            ...
            <input type="text" wire:model="kode" class="border rounded px-2 py-1 w-full" required />
            @error('kode') <span class="text-red-500">{{ $message }}</span> @enderror
        </div>
        <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded">Simpan</button>
        <a href="{{ route('reff-infeksi-oportunistik.index') }}" class="ml-2 text-gray-600">Batal</a>
    </form>
</div>
