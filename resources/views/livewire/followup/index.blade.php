...
<div>
    <div class="mb-6 flex justify-between items-center">
        ...
        <a href="{{ route('followup.create', ['id_pasien' => null]) }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg">Tambah Follow Up</a>
    </div>
    <div class="mb-6">
            ...
            <tbody>
                @forelse($followups as $index => $followup)
                    <tr class="border-b">
                        ...
                        <td class="px-6 py-4">
                            <a href="{{ route('followup.show', $followup->id_riwayat_perawatan_pasien) }}" class="text-blue-600">Detail</a>
                            <a href="{{ route('followup.edit', $followup->id_riwayat_perawatan_pasien) }}" class="text-yellow-600 ml-2">Edit</a>
                            @if(auth()->user()->type === 'admin')
                                <button wire:click="delete({{ $followup->id_riwayat_perawatan_pasien }})" class="text-red-600 ml-2">Hapus</button>
                            @endif
                        </td>
                    </tr>
                @empty
