<div>
    <div class="mb-4 flex flex-wrap items-center justify-between gap-2">
        <h2 class="text-xl font-semibold text-gray-800">Riwayat Terapi ART</h2>
        <a href="{{ route('riwayat-terapi-art.create') }}" class="px-3 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">+ Tambah</a>
    </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 border-b">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Jenis Terapi</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-gray-600">{{ $item->jenisTerapi->nama ?? '-' }}</td>
                        <td class="px-4 py-3 text-gray-600">{{ $item->paduanArt->nama ?? '-' }}</td>
                        <td class="px-4 py-3 text-gray-600">{{ $item->dosis_art ?? '-' }}</td>
                        <td class="px-4 py-3">
                            <a href="{{ route('riwayat-terapi-art.edit', $item->id_riwayat_terapi_art) }}" class="text-blue-600 hover:underline text-sm">Ubah</a>
                            <button wire:click="delete({{ $item->id_riwayat_terapi_art }})" wire:confirm="Yakin ingin menghapus data ini?" class="text-red-600 hover:underline text-sm ml-2">Hapus</button>
                        </td>
                    </tr>
                    @empty
