<div>
    <h2 class="text-xl font-bold mb-4">{{ $id_kabupaten ? 'Edit' : 'Tambah' }} Kabupaten/Kota</h2>
    ...
    <form wire:submit.prevent="save" class="max-w-lg">
        ...
        <div class="mb-4">
            ...
            <select wire:model="id_provinsi" class="border rounded px-3 py-2 w-full">
                <option value="">-- Pilih Provinsi --</option>
                @foreach($provinsis as $prov)
                    <option value="{{ $prov->id_provinsi }}">{{ $prov->nama }}</option>
                @endforeach
            </select>
            @error('id_provinsi') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
        </div>
        <div class="flex gap-2">
            <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">Simpan</button>
            <a href="{{ route('reff-kabupaten.index') }}" class="px-4 py-2 rounded border hover:bg-gray-100">Batal</a>
        </div>
    </form>
</div>
