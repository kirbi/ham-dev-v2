<div>
    <h2 class="text-xl font-bold mb-4">Referensi Status TB</h2>
    <input type="text" wire:model="search" placeholder="Cari nama..." class="border rounded px-2 py-1 mb-4" />
    <a href="{{ route('reff-status-tb.create') }}" class="bg-blue-500 text-white px-4 py-2 rounded mb-4 inline-block">Tambah Data</a>
    <table class="min-w-full bg-white">
        ...
        <tbody>
            @foreach($statusTbs as $i => $item)
            <tr>
                <td class="py-2">{{ $statusTbs->firstItem() + $i }}</td>
