<div>
    ...
    <form wire:submit.prevent="save">
        ...
        <div class="mb-4">
            ...
            <select wire:model="tipe_tb_id" class="border rounded px-2 py-1 w-full" required>
                <option value="">Pilih Tipe TB</option>
                @foreach($tipeTbs as $tipe)
                    <option value="{{ $tipe->id }}">{{ $tipe->nama }}</option>
                @endforeach
            </select>
            @error('tipe_tb_id') <span class="text-red-500">{{ $message }}</span> @enderror
        </div>
        <div class="mb-4">
            <label class="block mb-1">Klasifikasi TB</label>
            <select wire:model="klasifikasi_tb_id" class="border rounded px-2 py-1 w-full" required>
            ...
            @error('klasifikasi_tb_id') <span class="text-red-500">{{ $message }}</span> @enderror
        </div>
        <div class="mb-4">
            <label class="block mb-1">Panduan TB</label>
            <select wire:model="paduan_tb_id" class="border rounded px-2 py-1 w-full" required>
                <option value="">Pilih Panduan TB</option>
                @foreach($paduanTbs as $paduan)
                    <option value="{{ $paduan->id }}">{{ $paduan->nama }}</option>
                @endforeach
            </select>
            @error('paduan_tb_id') <span class="text-red-500">{{ $message }}</span> @enderror
        </div>
        <div class="mb-4">
            <label class="block mb-1">Kabupaten Pengobatan</label>
            <select wire:model="kabupaten_id_pengobatan" class="border rounded px-2 py-1 w-full" required>
                <option value="">Pilih Kabupaten</option>
                @foreach($kabupatens as $kabupaten)
                    <option value="{{ $kabupaten->kabupaten_id }}">{{ $kabupaten->nama }}</option>
                @endforeach
            </select>
            @error('kabupaten_id_pengobatan') <span class="text-red-500">{{ $message }}</span> @enderror
        </div>
        <div class="mb-4">
