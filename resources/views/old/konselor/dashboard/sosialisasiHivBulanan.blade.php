        ...
        <div class="card">
            <div class="row">
                <div class="col-12 d-flex justify-content-center">
                    <h2>GRAFIK KEGIATAN SOSIALISASI HIV</h2>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="row justify-content-center">
            ...
            <div class="col-12 col-md-6">
                <div class="card">
                    <div class="card-header border-0">
                        <div class="d-flex justify-content-between">
                            <h3 class="card-title">Kegiatan SOSIALISASI HIV <b class="tahun"></b></h3>
                        </div>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="d-flex flex-row justify-content-end">
                            <span class="mr-2">
                                <i class="fas fa-square text-danger"></i> Total
                            </span>
                        </div>
                    </div>
                </div>
...
<script type="text/javascript">
    $(function() {
        ...
        $.ajax({
            type: "GET",
            url: "{{ route('konselor.dashboard.sosialisasiHivBulanan') }}",
            success: function(response) {
                salesChart.data.datasets[0].data = response.data.total;
        ...
        function tablecheck(data){
            $('#table-SOSIALISASI').html('');
            var i = 0;
            var bulan =1;
            for(i; i<data.data['total'].length; i++){
                $('#table-SOSIALISASI').append("<tr><td>"+bulan+"</td><td>"+data.data.total[i]+"</td></tr>");
                bulan++;
            }
        }
        ...
        $('.filter').click(function (e) {
            ...
            $.ajax({
                ...
                url: "{{ route('konselor.dashboard.sosialisasiHivBulanan.filter') }}",
                type: "POST",
                dataType: 'json',
                success: function(response) {
                    salesChart.data.datasets[0].data = response.data.total;
