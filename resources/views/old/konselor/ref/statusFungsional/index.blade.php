            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="productForm" name="productForm" class="form-horizontal">
                        <input type="hidden" name="id_status_fungsional" id="id_status_fungsional">
                        <div class="form-group">
                            <label for="kode" class="col-sm-2 control-label">Kode</label>
                            <div class="col-sm-12">
                                <input type="number" class="form-control" id="kode" name="kode" placeholder="Kode" value="" maxlength="10" required="">
                            </div>
                        </div>
                        <div class="form-group">
...
$(function () {
     ...
     var table = $('.data-table').DataTable({
         processing: true,
         serverSide: true,
         ajax: "{{ route('statusFungsional.index') }}",
         columns: [
     ...
     $('body').on('click', '.editProduct', function () {
         var id_status_fungsional = $(this).data('id');
         $.get("{{ route('statusFungsional.index') }}" +'/' + id_status_fungsional +'/edit', function (data) {
     ...
     $('#saveBtn').click(function (e) {
         ...
         $.ajax({
             ...
             url: "{{ route('statusFungsional.store') }}",
             type: "POST",
             dataType: 'json',
     ...
     $('body').on('click', '.deleteProduct', function (){
         ...
         if(result){
             $.ajax({
                 type: "DELETE",
                 url: "{{ route('statusFungsional.store') }}"+'/'+product_id,
                 success: function (data) {
                    if($.isEmptyObject(data.error)){
                        $(".alert-success").css('display','block');
