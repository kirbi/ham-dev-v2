            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="productForm" name="productForm" class="form-horizontal">
                        <input type="hidden" name="id_kategori_manfaat" id="id_kategori_manfaat">
...
$(function () {
     ...
     var table = $('.data-table').DataTable({
         processing: true,
         serverSide: true,
         ajax: "{{ route('kategoriManfaat.index') }}",
         columns: [
     ...
     $('body').on('click', '.editProduct', function () {
         var id_kategori_manfaat = $(this).data('id');
         $.get("{{ route('kategoriManfaat.index') }}" +'/' + id_kategori_manfaat +'/edit', function (data) {
             $('#modelHeading').html("Ubah Data");
             $('#saveBtn').val("edit-user");
             $('#ajaxModel').modal('show');
             $('#nama').val(data.nama);
             $('#id_kategori_manfaat').val(data.id_kategori_manfaat);
     ...
     $('#saveBtn').click(function (e) {
         ...
         $.ajax({
             ...
             url: "{{ route('kategoriManfaat.store') }}",
             type: "POST",
             dataType: 'json',
     ...
     $('body').on('click', '.deleteProduct', function (){
         ...
         if(result){
             $.ajax({
                 type: "DELETE",
                 url: "{{ route('kategoriManfaat.store') }}"+'/'+product_id,
                 success: function (data) {
                    if($.isEmptyObject(data.error)){
                        $(".alert-success").css('display','block');
