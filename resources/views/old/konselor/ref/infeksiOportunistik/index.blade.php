            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="productForm" name="productForm" class="form-horizontal">
                        <input type="hidden" name="id_infeksi_oportunistik" id="id_infeksi_oportunistik">
                        <div class="form-group">
                            <label for="kode" class="col-sm-2 control-label">Kode</label>
                            <div class="col-sm-12">
                                <input type="text" class="form-control" id="kode" name="kode" placeholder="Kode" value="" maxlength="10" required="">
                            </div>
                        </div>
                        <div class="form-group">
...
$(function () {
     ...
     var table = $('.data-table').DataTable({
         processing: true,
         serverSide: true,
         ajax: "{{ route('infeksiOportunistik.index') }}",
         columns: [
     ...
     $('body').on('click', '.editProduct', function () {
         var id_infeksi_oportunistik = $(this).data('id');
         $.get("{{ route('infeksiOportunistik.index') }}" +'/' + id_infeksi_oportunistik +'/edit', function (data) {
             $('#modelHeading').html("Ubah Data");
             $('#saveBtn').val("edit-user");
             $('#ajaxModel').modal('show');
             $('#nama').val(data.nama);
             $('#id_infeksi_oportunistik').val(data.id_infeksi_oportunistik);
     ...
     $('#saveBtn').click(function (e) {
         ...
         $.ajax({
             ...
             url: "{{ route('infeksiOportunistik.store') }}",
             type: "POST",
             dataType: 'json',
     ...
     $('body').on('click', '.deleteProduct', function (){
         ...
         if(result){
             $.ajax({
                 type: "DELETE",
                 url: "{{ route('infeksiOportunistik.store') }}"+'/'+product_id,
                 success: function (data) {
                    if($.isEmptyObject(data.error)){
                        $(".alert-success").css('display','block');
