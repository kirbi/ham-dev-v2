...
$(function () {
    ...
    $('.select2').select2();
     ...
     var table = $('.data-table').DataTable({
         ...
         lengthChange: false,
         autoWidth: false,
         ajax: "{{ route('sosialisasiHiv.index') }}",
         columns: [
     ...
     $('body').on('click', '.editProduct', function () {
         ...
         $.get("{{ route('sosialisasiHiv.index') }}" +'/' + id_sosialisasi_hiv +'/edit', function (data) {
     ...
     $('#saveBtn').click(function (e) {
         ...
         $.ajax({
             ...
             url: "{{ route('sosialisasiHiv.store') }}",
             type: "POST",
             dataType: 'json',
     ...
     $('body').on('click', '.deleteProduct', function (){
         ...
         if(result){
             $.ajax({
                 type: "DELETE",
                 url: "{{ route('sosialisasiHiv.store') }}"+'/'+product_id,
                 success: function (data) {
                    if($.isEmptyObject(data.error)){
                        $(".alert-success").css('display','block');
